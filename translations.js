const translations = {
  ja: {
    // Navigation
    sierraJapan: "シエラ・ジャパン・エンタープライズ",
    nepaliDining: "ネパール料理",
    baralEngineering: "バラルエンジニアリング",
    sastoSulav: "サストスラブエクスプレス",
    about: "について",
    close: "閉じる",

    // HOME PAGE
    knowMore: "もっと知る",
    groceries: "食料品",
    technology: "テクノロジー",
    exploreFeaturedTopics: "注目のトピックを調べる",
    exploreMore: "もっと詳しく見る",
    servingCommunity: "長年にわたりコミュニティに貢献しています。",

    // HOME CONTENT DESCRIPTIONS
    baralEngineeringDesc:
      "バラルエンジニアリングは、革新的なITソリューションとソフトウェア開発をリードしています。カスタムソフトウェア開発、モバイルアプリケーション、そしてビジネス成長を促進するエンタープライズソリューションを専門としています。熟練したエンジニアチームが、技術的専門知識と創造的な問題解決を組み合わせて、クライアントに最先端のソリューションを提供しています。",
    sastoSulavDesc:
      "日本における本格的な南アジアの食材を提供する一流のデスティネーション。ネパールと南アジアの味を直接お届けし、スパイス、豆類、米、伝統的な食材を幅広く取り揃えています。当社のエクスプレス配送サービスにより、新鮮で品質の高い商品を迅速にお届けします。",
    nepaliDiningDesc:
      "日本の中心部で本場のネパール料理をお楽しみください。当レストランでは、熟練シェフが伝統的なネパール料理を提供し、風味とスパイスの完璧な調和を実現しています。定番のモモから濃厚なカレーまで、ネパールの温かさと親しみやすさを食事体験に取り入れています。",
    sierraJapanDesc:
      "シエラジャパンエンタープライズは、日本とネパールの文化とビジネスの架け橋となっています。包括的なビジネスコンサルティング、市場参入戦略、文化交流プログラムを提供しています。私たちの専門知識は、意味のある異文化間の繋がりを育みながら、企業が国際市場を開拓するのを支援します。",

    // NEPALI DINING PAGE
    nepali: "ネパール",
    cuisines: "料理",
    seeMenu: "メニューを見る",
    ourFacilities: "施設のご案内",
    hotDeals: "お得な情報",
    hotDealsDesc: "常にお客様へのお得な情報をご用意しております。",
    amazingFood: "美味しい料理",
    amazingFoodDesc: "最高の味と品質を提供いたします。",
    fastDelivery: "迅速な配達",
    fastDeliveryDesc: "素早く確実な配達サービスを提供いたします。",
    gallery: "ギャラリー",
    ourMenu: "メニュー",
    seasonal: "季節の食材を使用",
    appetizers: "前菜",
    aboutUs: "私たちについて",
    aboutDescription:
      "私たちは20年以上にわたって本格的なネパール料理を提供している家族経営のレストランです。私たちの使命は、お客様に温かく居心地の良い雰囲気、美味しい料理、そして卓越したサービスを提供することです。新鮮な食材と伝統的な調理法にこだわり、心を込めて料理を作っています。気軽な一品からゆっくりとしたお食事まで、ネパールの味をぜひご体験ください。ご来店をお待ちしております。",

    // SIERRA JAPAN ENTERPRISES PAGE
    exploreWonders: "旅行と観光の素晴らしさを探る",
    realComfort: "本当の快適さ、視覚的にも物理的にも、すべての部屋に不可欠です",
    aroundWorld:
      "世界一周80日間で、虹の終わりにある金の壺を見つけるでしょう。時間に対して戦い、風の白い翼で飛んでいきます。",
    welcomeRestaurant:
      "レストランガイドへようこそ！ここでは、あなたの街で最高のダイニングスポットを厳選してご紹介しています。居心地の良いカフェ、高級レストラン、軽食など、あなたが探しているものが何であれ、私たちがサポートします。私たちのおすすめを探索して、周辺の一流レストランで美味しい食事をお楽しみください。",
    cozyCafes: "温かい雰囲気の居心地の良いカフェ",
    fineDining: "特別な機会のための高級レストラン",
    quickBites: "素早く美味しい食事のための軽食",
    familyFriendly: "子供向けの素晴らしいメニューがある家族向けの場所",
    hiddenGems: "地元の人だけが知る隠れた名店",

    // BARAL ENGINEERING PAGE
    discoverLatest:
      "エンジニアリングとテクノロジーの最新イノベーションとトレンドを発見する",
    preventCostly: "コストのかかるミスを防止",
    mistakes: "ミス",
    createTemplates:
      "事前承認済みのテンプレートを作成し、すべての法的情報をロックします。",
    letsConnect: "お問い合わせ",
    ourTopFacilities: "主な施設",
    hipaaCompliant: "HIPAAおよびSOC2準拠",
    hipaaDesc:
      "当社のアプリケーションはHIPAAおよびSOC2に準拠しており、お客様のデータは常に安全です。",
    socialMedia: "自動ソーシャルメディア投稿",
    socialMediaDesc:
      "複数のプラットフォームにわたるソーシャルメディア投稿をスケジュールおよび自動化して、時間を節約し、一貫したオンラインプレゼンスを維持します。",
    analytics: "高度な分析",
    analyticsDesc:
      "詳細な分析とレポートツールを使用してソーシャルメディアのパフォーマンスに関する洞察を得て、エンゲージメントとROIを測定します。",
    calendar: "コンテンツカレンダー",
    calendarDesc:
      "直感的なカレンダービューでソーシャルメディアコンテンツを計画および整理し、投稿を見逃さないようにします。",
    audience: "オーディエンスターゲティング",
    audienceDesc:
      "人口統計、興味、行動などの高度なターゲティングオプションで適切なオーディエンスにリーチします。",
    listening: "ソーシャルリスニング",
    listeningDesc:
      "ソーシャルメディアの会話やトレンドをモニタリングして、オーディエンスが何を言っているかを把握し、リアルタイムで対応します。",
    whoAreWe: "私たちについて",
    aboutBaralEng:
      "バラルエンジニアリングは、革新的な技術ソリューションと先進的なエンジニアリングサービスを専門とする先進的なエンジニアリング企業です。卓越性への取り組みを基に設立され、ソフトウェア開発、システム統合、技術コンサルティングの専門知識を結集して、日本全国のビジネスに包括的なソリューションを提供しています。熟練したエンジニアと技術専門家のチームは、カスタムソフトウェアアプリケーションの作成、自動化システムの実装、組織が業務を最適化し、今日のデジタル環境で競争力を維持するのに役立つ技術サポートの提供に優れています。私たちは、複雑な技術的課題を理解し、ビジネスの成長とイノベーションを促進する効率的でスケーラブルなソリューションを開発する能力を誇りにしています。品質、信頼性、最先端の技術に焦点を当て、バラルエンジニアリングは、技術を活用して成功を収めようとするビジネスの信頼できるパートナーであり続けています。",

    // ABOUT PAGE
    baralBrothers: "バラル＆ブラザーズ",
    whoWeAre: "私たちは誰ですか？",
    whoWeAreDesc:
      "バラル＆ブラザーズは、日本で設立された多様なビジネスを一つの傘下に集めたダイナミックな複合企業です。文化の架け橋を築き、価値を創造するというビジョンを持って設立され、小さな家族経営のビジネスから、小売、飲食、エンジニアリング、輸出入サービスなど様々な分野で尊敬される名前へと成長してきました。",
    whatWeDo: "私たちは何をしていますか？",
    whatWeDoDesc:
      "私たちは4つの異なるビジネスを運営しています：輸出入を専門とするシエラジャパンエンタープライズ、本格的な料理を提供するネパールダイニング、技術的ソリューションを提供するバラルエンジニアリング、そして高品質の食料品を届けるサストスラブエクスプレスです。各事業は、卓越性への私たちのコミットメントと、多様で質の高いサービスで日本市場を豊かにするという目標を反映しています。",
    followUs: "フォローする",

    // SASTO SULAV EXPRESS PAGE
    shopNow: "今すぐ購入",
    sastoSulavExpress: "サストスラブエクスプレス",
    sastoSulavDesc2:
      "サストスラブは、日本での便利でお手頃価格のワンストップ食料品店です。新鮮な農産物、高品質の商品、そして比類のない価格を提供し、あなたの買い物をより便利で手頃な価格にします。",
    featuredProducts: "おすすめ商品",
    about: "について",
    aboutSastoSulav:
      "Sasto Sulav Expressでは、高品質な商品をお求めやすく、お手頃な価格でご提供することを信条としています。お客様満足への取り組みは、商品の提供だけにとどまりません。競争力のある価格、信頼性の高い配送サービス、そして商品選びに関する専門家のアドバイスもご提供いたします。日用品から、お祭りやお祝いのための特別な商品まで、豊富な在庫でお客様のニーズにぴったりの商品がきっと見つかります。ぜひご来店いただき、お気に入りの南アジア食材を日本で手軽にお買い求めください。",
    location: "場所",
    japan: "日本、アジア",
    sastoSulavWelcome:
      "Sasto Sulav Expressへようこそ。日本国内で本格的な南アジア食材をお届けする信頼できるお店です。ネパール、インド、そして周辺地域から直輸入した高品質な商品を豊富に取り揃えており、その誇りを誇りに思っています。スパイス、レンズ豆、米、生鮮食品、伝統的なスナックなど、南アジア料理に欠かせない食材を専門に取り扱っています。故郷の味を味わうことの大切さを理解し、地域社会のニーズに応えるべく、厳選した商品をお届けしています。",
    sastoSulavCardDesc:
      "Sasto Sulavは、新鮮な食料品、高品質な製品、そして日用品を日本国内で提供しています。手頃な価格と利便性で、すべてのお客様に快適なショッピング体験を提供します。",
    groceryStore: "食料品店",
  },
  en: {
    // Navigation
    sierraJapan: "Sierra Japan Enterprises",
    nepaliDining: "Nepali Dining",
    baralEngineering: "Baral Engineering",
    sastoSulav: "Sasto Sulav Express",
    about: "ABOUT",
    close: "Close",

    // HOME PAGE
    knowMore: "Know more",
    groceries: "Groceries",
    technology: "Technology",
    exploreFeaturedTopics: "Explore Featured Topics",
    exploreMore: "Explore more",
    servingCommunity: "Serving the community for years.",

    // HOME CONTENT DESCRIPTIONS
    baralEngineeringDesc:
      "Baral Engineering leads the way in innovative IT solutions and software development. We specialize in custom software development, mobile applications, and enterprise solutions that drive business growth. Our team of skilled engineers combines technical expertise with creative problem-solving to deliver cutting-edge solutions for our clients.",
    sastoSulavDesc:
      "Your premier destination for authentic South Asian groceries in Japan. We bring the flavors of Nepal and South Asia directly to your doorstep, offering a wide selection of spices, lentils, rice, and traditional ingredients. Our express delivery service ensures fresh and quality products reach you quickly.",
    nepaliDiningDesc:
      "Experience the authentic taste of Nepal in the heart of Japan. Our restaurant serves traditional Nepali cuisine prepared by expert chefs, offering a perfect blend of flavors and spices. From classic momos to rich curries, we bring the warmth and hospitality of Nepal to your dining experience.",
    sierraJapanDesc:
      "Sierra Japan Enterprises bridges cultural and business gaps between Japan and Nepal. We provide comprehensive business consulting, market entry strategies, and cultural exchange programs. Our expertise helps businesses navigate international markets while fostering meaningful cross-cultural connections.",

    // NEPALI DINING PAGE
    nepali: "Nepali",
    cuisines: "Cuisines",
    seeMenu: "see menu",
    ourFacilities: "Our Facilities",
    hotDeals: "Hot Deals & Offers",
    hotDealsDesc: "We always have hot deals and offers for our customers.",
    amazingFood: "Amazing Food",
    amazingFoodDesc: "We serve high-quality authentic Nepali cuisine.",
    fastDelivery: "Fast Delivery",
    fastDeliveryDesc: "Quick and reliable delivery service to your doorstep.",
    gallery: "Gallery",
    ourMenu: "Our Menu",
    seasonal: "Seasonal & Locally Sourced Ingredients",
    appetizers: "Appetizers",
    aboutUs: "About Us",
    aboutDescription:
      "We are a family-owned restaurant that has been serving authentic Nepali cuisine for over 20 years. Our mission is to provide our customers with a warm and welcoming atmosphere, delicious food, and exceptional service. We take pride in using only the freshest ingredients and traditional cooking methods to create our dishes. Whether you're looking for a quick bite or a sit-down meal, we hope you'll come and experience the flavors of Nepal with us. Thank you for choosing us!",

    // SIERRA JAPAN ENTERPRISES PAGE
    exploreWonders: "Explore the Wonders of Travel and Tourism",
    realComfort: "Real comfort, visual and physical, is vital to every room",
    aroundWorld:
      "80 days around the world, we'll find a pot of gold just sitting where the rainbow's ending. Time – we'll fight against the time, and we'll fly on the white wings of the wind.",
    welcomeRestaurant:
      "Welcome to our restaurant guide! Here, you will find a curated list of the best dining spots in your city. Whether you're looking for a cozy cafe, a fine dining experience, or a quick bite, we've got you covered. Explore our recommendations and enjoy delicious meals at the top restaurants around you.",
    cozyCafes: "Cozy cafes with a warm ambiance",
    fineDining: "Fine dining restaurants for special occasions",
    quickBites: "Quick bites for a fast and tasty meal",
    familyFriendly: "Family-friendly places with great menus for kids",
    hiddenGems: "Hidden gems known only to locals",

    // BARAL ENGINEERING PAGE
    discoverLatest:
      "DISCOVER THE LATEST IN ENGINEERING AND TECHNOLOGY INNOVATIONS AND TRENDS",
    preventCostly: "Prevent costly",
    mistakes: "mistakes",
    createTemplates:
      "Create pre-approved templates and lock all legal information.",
    letsConnect: "Let's Connect",
    ourTopFacilities: "Our Top Facilities",
    hipaaCompliant: "HIPAA and SOC2 Compliant",
    hipaaDesc:
      "Our applications are HIPAA and SOC2 compliant, your data is safe with us, always.",
    socialMedia: "Automated Social Media Posting",
    socialMediaDesc:
      "Schedule and automate your social media posts across multiple platforms to save time and maintain a consistent online presence.",
    analytics: "Advanced Analytics",
    analyticsDesc:
      "Gain insights into your social media performance with detailed analytics and reporting tools to measure engagement and ROI.",
    calendar: "Content Calendar",
    calendarDesc:
      "Plan and organize your social media content with an intuitive calendar view, ensuring you never miss a post.",
    audience: "Audience Targeting",
    audienceDesc:
      "Reach the right audience with advanced targeting options, including demographics, interests, and behaviors.",
    listening: "Social Listening",
    listeningDesc:
      "Monitor social media conversations and trends to stay informed about what your audience is saying and respond in real-time.",
    whoAreWe: "Who are we?",
    aboutBaralEng:
      "Baral Engineering is a forward-thinking engineering firm specializing in innovative technological solutions and advanced engineering services. Founded with a commitment to excellence, we bring together expertise in software development, system integration, and technical consulting to deliver comprehensive solutions for businesses across Japan. Our team of skilled engineers and technology experts excels in creating custom software applications, implementing automation systems, and providing technical support that helps organizations optimize their operations and stay competitive in today's digital landscape. We pride ourselves on our ability to understand complex technical challenges and develop efficient, scalable solutions that drive business growth and innovation. With a focus on quality, reliability, and cutting-edge technology, Baral Engineering continues to be a trusted partner for businesses seeking to leverage technology for their success.",

    // ABOUT PAGE
    baralBrothers: "Baral & Brothers",
    whoWeAre: "Who are we?",
    whoWeAreDesc:
      "Baral & Brothers is a dynamic multi-business enterprise established in Japan, bringing together diverse ventures under one umbrella. Founded with a vision to bridge cultures and create value, we've grown from a small family business into a respected name in various sectors including retail, dining, engineering, and import-export services.",
    whatWeDo: "What we do?",
    whatWeDoDesc:
      "We operate four distinct businesses: Sierra Japan Enterprises specializing in import-export, Nepali Dining offering authentic cuisine, Baral Engineering providing technical solutions, and Sasto Sulav Express delivering quality groceries. Each venture reflects our commitment to excellence and our goal of enriching the Japanese market with diverse, quality services.",
    followUs: "Follow us on",

    // SASTO SULAV EXPRESS PAGE
    shopNow: "Shop Now",
    sastoSulavExpress: "Sasto Sulav Express",
    sastoSulavDesc2:
      "Sasto Sulav, your one-stop grocery store in Japan, offers fresh produce, quality goods, and unbeatable prices to make every shopping trip convenient and affordable for you.",
    featuredProducts: "Featured Products",
    about: "About",
    sastoSulavWelcome:
      "Welcome to Sasto Sulav Express, your trusted source for authentic South Asian groceries in Japan. We take pride in offering a diverse selection of high-quality products imported directly from Nepal, India, and surrounding regions. Our store specializes in essential ingredients for South Asian cuisine,  including spices, lentils, rice, fresh produce, and traditional snacks. We understand the importance of finding familiar tastes from home, which is why we carefully curate our product selection to meet the needs of our community.",
    aboutSastoSulav:
      "At Sasto Sulav Express, we believe in making quality products accessible and affordable. Our commitment to customer satisfaction extends beyond just providing products - we offer competitive prices, reliable delivery services, and expert advice on product selection. Whether you're looking for everyday essentials or specialty items for festivals and celebrations, our extensive inventory ensures you'll find exactly what you need. Visit our store today and experience the convenience of shopping for your favorite South Asian groceries in Japan. ",
    sastoSulavCardDesc:
      "Sasto Sulav provides fresh groceries, quality products, and everyday essentials in Japan. Offering affordability and convenience, it ensures a delightful shopping experience for all customers.",
    groceryStore: "Grocery Store",
  },
};
